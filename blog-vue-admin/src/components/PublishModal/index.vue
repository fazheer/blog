...
watch(()=>props.title,()=>{
  article.value.title=props.title
})
watch(()=>props.content,()=>{
  article.value.content=props.content
})
...
const PublishArticle = () => {
  article.value.createAt = moment(new Date()).format("yyyy-MM-DD HH:mm:ss")
  article.value.categoryName=categoryName.value
  article.value.tags=Array.from(tags.value)
  api.publishArticle(article.value).then(({data: data}) => {
    if (data.code == 10000) {
...
const handleUploadFinish = (options: {
  ...
  if (options.file.status == 'finished') {
    ...
    article.value.coverUrl =data.data
...
<template>
  <n-modal :show="modalShow.showPublish">
    <n-card
        style="width: 600px"
        title="发布文章"
        :bordered="false"
    >
      ...
      <template #footer>
        <div class="footer">
          <n-space>
            <n-button type="tertiary" @click="modalShow.showPublish=false">取消</n-button>
            <n-button type="error" @click="PublishArticle">发布</n-button>
          </n-space>
        </div>
      </template>
    </n-card>
